...
/*enter my code here*/
template <typename Dtype>
void csr2csc(const int A_num_row, const int A_num_col, 
        const int* row_ptr, const int* col_index, const Dtype* csr_val,
        int* col_ptr, int* row_index, Dtype* csc_val)
{  //convert CSR format to CSC format
    const int nnz = row_ptr[A_num_row];

    //compute number of non-zero entries per column of A 
    std::fill(col_ptr, col_ptr + A_num_col, 0);

    for (int n = 0; n < nnz; n++){            
        col_ptr[col_index[n]]++;
    }

    //cumsum the nnz per column to get col_ptr[]
    for(int col = 0, cumsum = 0; col < A_num_col; col++){     
        int temp  = col_ptr[col];
        col_ptr[col] = cumsum;
        cumsum += temp;
    }
    col_ptr[A_num_col] = nnz; 

    for(int row = 0; row < A_num_row; row++){
        for(int jj = row_ptr[row]; jj < row_ptr[row+1]; jj++){
            int col  = col_index[jj];
            int dest = col_ptr[col];
    ...
    for(int col = 0, last = 0; col <= A_num_col; col++){
        int temp  = col_ptr[col];
        col_ptr[col] = last;
        last = temp;
    }
...
template <typename Dtype>
int num_of_nonzeros(const Dtype* input, int rows, int columns)
{//return the number of nonzeros in input
	int num = 0;
	for(int col = 0; col < columns; col++)
	{
		for(int row = 0; row < rows; row++)
		{
			if(input[row*columns+col]!=(Dtype)0.0)
				num++;
		}
	}
	return num;
...
template <typename Dtype>
int weight2csc(const Dtype* weights, int rows, int columns, 
    int* row_index,Dtype* val,int* col_ptr)
{//return the number of nonzeros in weights, and convert weights ro CSC format
  col_ptr[0] = 0;
  int num = 0;
  ...
  for(int col = 0; col < columns; col++)
  {
    for(int row = 0; row < rows; row++)
    {
      if(weights[row*columns+col]!=(Dtype)0.0)
      {
        val[num] = weights[row*columns+col];
        row_index[num] = row;
        num++;
      }
    }
...
template <typename Dtype>
int input2csr(const Dtype* input, int rows, int columns, 
		int* col_index, Dtype* val,int* row_ptr)
{//return the number of nonzeros in input, and convert input ro CSR format
	row_ptr[0] = 0;
	int num = 0;
	for(int row = 0; row < rows; row++)
	{
...
template <typename Dtype>
void CSC_MUL_CSR(int* row_index,Dtype* csc_val,int* col_ptr, int col_ptr_size,
		int* col_index,Dtype* csr_val,int* row_ptr, int row_ptr_size,
		Dtype* output, int o_width)
{
	for(int rown = 0; rown < row_ptr_size-1; rown++)
	{
		for(int c = row_ptr[rown];c < row_ptr[rown+1];c++)
		{
			for(int r = col_ptr[rown]; r < col_ptr[rown+1]; r++)
				output[row_index[r]*o_width+col_index[c]] += csc_val[r] * csr_val[c];
		}
	}
}

template <typename Dtype>
Dtype* init_buf(const int row, const int column, Dtype sparsity, Dtype max){
  Dtype* buf = new Dtype[row*column];
  for (int i=0;i<row*column;i++){
    buf[i] = (Dtype)((sparsity < drand48()) ? (max - 2*max*drand48()) : 0.0);
  }
  return buf;
}

/*enter my code here*/
int flag=0;double input2csr_totaltime=0;double sparse_train_time=0; double origin_time=0;
template <typename Dtype>
void BaseConvolutionLayer<Dtype>::forward_cpu_gemm(const Dtype* input,
  ...
  /*enter my code here*/
  if (this->layer_param_.convolution_param().conv_mode() == caffe::ConvolutionParameter_ConvMode_CSCCSR_MUL)
  	memset(output,0,sizeof(Dtype)*M*N);
  /*enter my code here*/
  for (int g = 0; g < group_; ++g) {
	  const int row_offset = conv_out_channels_ /group_ + 1;
	  switch(this->layer_param_.convolution_param().conv_mode()){
    /*enter my code here*/
    case caffe::ConvolutionParameter_ConvMode_CSCCSR_MUL: {
      //M: Number of rows in matrices weight and output.
      //N: Number of columns in matrices col_buff and output.
      //K: Number of columns in matrix weight; number of rows in matrix col_buff.
      //const Dtype* made_input = init_buf(K, N, (Dtype)0.9, (Dtype)25.0);
      //const Dtype* made_weight = init_buf(M, K, (Dtype)0.9, (Dtype)0.2);
      double start, stop;
      //start = clock();
      const int weight_num_nonzeros = num_of_nonzeros(weights/*made_weight*/,M,K);	//the number of values of weight
      //stop = clock();
      //totaltime += ((double)(stop-start))/CLOCKS_PER_SEC;
      //printf("Count weight sparsity: %f\n",totaltime);
      //find the the number of nonzeros in input, and calculate the sparsity
      //start = clock();
      const int input_num_nonzeros = num_of_nonzeros(col_buff/*made_input*/,K,N);
      //stop = clock();
      //totaltime += ((double)(stop-start))/CLOCKS_PER_SEC;
      //printf("Count input sparsity: %f\n",totaltime);
      LOG(INFO)<<"The sparsity of input is: "<<1-input_num_nonzeros/(1.0*K*N);
      LOG(INFO)<<"The sparsity of weight is: "<<1-weight_num_nonzeros/(1.0*K*M);
      //convert input to CSR format first
      start = clock();
      Dtype* input_val = new Dtype[input_num_nonzeros];
      int* input_colidx = new int[input_num_nonzeros];
      int* input_rowptr = new int[K+1];// remember to delete them later!!!
      input2csr(col_buff/*made_input*/, K, N, input_colidx, input_val, input_rowptr);
      stop = clock();
      input2csr_totaltime += ((double)(stop-start))/CLOCKS_PER_SEC;
      printf("input2csr time: %f\n",input2csr_totaltime);
      //convert weight to CSC format
      //start = clock();
      Dtype* weight_val = new Dtype[weight_num_nonzeros];
      int* weight_rowidx = new int[weight_num_nonzeros];
      int* weight_colptr = new int[K+1];
      weight2csc(weights/*made_weight*/, M, K, weight_rowidx, weight_val, weight_colptr);
      //stop = clock();
      //totaltime += ((double)(stop-start))/CLOCKS_PER_SEC;
      //printf("weight2csc time: %f\n",totaltime);
      const int input_offset = col_offset_/group_;
      const int col_offset = conv_out_spatial_dim_ / group_ + 1;
      start = clock();
      CSC_MUL_CSR(weight_rowidx + weight_offset_ * g, weight_val + weight_offset_ * g, weight_colptr + row_offset * g, K+1,
		      input_colidx + input_offset * g, input_val + input_offset * g, input_rowptr + col_offset * g, K+1,
		      output + output_offset_ * g, N);
      stop = clock();
      sparse_train_time += ((double)(stop-start))/CLOCKS_PER_SEC;
      printf("Sparse mul time: %f\n",sparse_train_time);

      Dtype* temp_output = new Dtype[M*N];
      memset(temp_output,0,sizeof(Dtype)*M*N);
      start = clock();
      caffe_cpu_gemm<Dtype>(CblasNoTrans, CblasNoTrans, M, N, K,
             (Dtype)1., weights + weight_offset_ * g, col_buff + col_offset_ * g,
             (Dtype)0., temp_output + output_offset_ * g);
      stop = clock();
      origin_time += ((double)(stop-start))/CLOCKS_PER_SEC;
      printf("Origin multiple time: %f\n",origin_time);

/*
      if(flag==0){
        printf("the input_colidx:\n" );
        for(int i=0;i<input_num_nonzeros;i++)
          printf("%d ", input_colidx[i]);
        printf("\n");
        printf("the weight_rowidx:\n" );
        for(int i=0;i<weight_num_nonzeros;i++)
          printf("%d ", weight_rowidx[i]);
        printf("\n");
        flag=1;
     }
*/
...
/*
      if(flag==0){
        ...
        for(int i=0;i<M*K;i++)
          printf("%f ", weights[i]);
        printf("\n");
        printf("the weight_colptr:\n" );
        for(int i=0;i<weight_num_nonzeros;i++)
          printf("%f ", weight_val[i]);
        printf("\n");
        flag=1;
     }
*/
/*
      ...
      printf("N: %d\n", N);
      printf("K: %d\n", K);
      printf("the input_rowptr:\n" );
      for(int i=0;i<K+1;i++)
        printf("%d ", input_rowptr[i]);
      printf("\n");
      printf("the weight_colptr:\n" );
      for(int i=0;i<K+1;i++)
        printf("%d ", weight_colptr[i]);
      printf("\n");
*/
/*
      if(flag==0){
        printf("the output I calculate:\n" );
        for(int i=0;i<N*M;i++)
          printf("%f ", output[i]);
        printf("\n");
      }
...
//        memset(temp_output,0,sizeof(Dtype)*M*N);
//        caffe_cpu_gemm<Dtype>(CblasNoTrans, CblasNoTrans, M, N, K,
//             (Dtype)1., /*weights*/made_weight + weight_offset_ * g, /*col_buff*/made_input + col_offset_ * g,
//             (Dtype)0., temp_output + output_offset_ * g);
//         //printf("the true output:\n" );
//         //for(int i=0;i<N*M;i++)
//         //  printf("%f ", temp_output[i]);
//         //printf("\n");
//        for(int i=0;i<N*M;i++){
//          if(output[i]==temp_output[i])
//            printf("SAME\n");
//          else
//            printf("DIFFERENT!!!: i=%d, output=%f, true=%f\n",i,output[i],temp_output[i] );
//        }
//         delete[] temp_output;
//         flag=1;
//      }
      ...
      delete[] input_val;
  	  ...
  	  delete[] input_colidx;
  	  delete[] weight_val;
  	  delete[] weight_rowidx;
  	  delete[] weight_colptr;
      break;
  	  }
    ...
    /*enter my code here*/
      ...
      printf("N: %d\n", N);
      printf("K: %d\n", K);
      printf("weight_offset_: %d\n", weight_offset_);
      printf("row_offset: %d\n", row_offset);
      printf("col_offset_: %d\n", col_offset_);
      printf("output_offset_: %d\n", output_offset_);
      printf("the weight_val:\n" );
      for(int i=0;i<nz_weight_values_.count();i++)
      {
        printf("%f ",nz_weight_values_.cpu_data()[i] );
      }
      printf("\n");*/
      /*
      for(int i=0;i<N*M;i++)
...
void BaseConvolutionLayer<Dtype>::backward_cpu_gemm(const Dtype* output,
  ...
  if(conv_param.conv_mode() != caffe::ConvolutionParameter_ConvMode_LOWERED_GEMM && 
      conv_param.conv_mode() != caffe::ConvolutionParameter_ConvMode_CSCCSR_MUL){
